...
namespace FPSDemo.Player
{
	public class PlayerWeaponMover : MonoBehaviour
	{
        ...
        {
            Vector3 recoilToAdd = _weaponController.EquippedWeapon.recoil;
		...
		private void ChangeGunPos(GunPosition newGunPos)
		{
			...
			switch (newGunPos)
			{
				...
				case GunPosition.aiming:
					_targetWeaponPos = _weaponController.EquippedWeapon.blockingPos;
					_targetWeaponRot = _weaponController.EquippedWeapon.blockingRot;
					_weaponController.WeaponAtTheReady = true;
					break;
		...
		{
			if (_player.IsClimbing || _player.MoveTowardsFinished == false)
			{
				ChangeGunPos(GunPosition.away);
			}
